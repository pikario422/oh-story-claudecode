...
直接说出结论，而不是让读者从细节中自己得出结论。

| 解释（禁止） | 描写（正确） |
|-------------|-------------|
| 他很疲惫 | 他靠着墙滑下去，鞋跟在地板上蹭出一道声音 |
| 气氛很尴尬 | 两个人都看着桌上的茶杯，没人说话 |
| 她对他有意思 | 她把手机翻出来看了三次，都是他的名字 |

**检测方法**：段落里出现形容词/状态词直接定性角色或场景 = 解释。改成能让读者"看见"的动作或细节。

### 模式 9：先否定再肯定句式

"不是A，是B"结构在AI写作中高频出现，读起来像分析报告而非小说叙事。
...
**处理原则**：偶尔一次可以，连续出现或用于总结段落时必须改写。用具体动作或对话替代这种"点题"式句子，让读者自己得出结论。

### 模式 10：破折号滥用

破折号（——）被AI用来制造"停顿感"或"强调感"，实际上是在用标点代替写作。

**高危用法**：
- 句末破折号收束（"他转过身——"）连续出现超过 2 次
- 用破折号代替正常句子结构（"她看着他——沉默——然后转身"）
- 段落内多个破折号制造"文学感"

**阈值**：每 500 字超过 2 个破折号 = 检查是否滥用。

**处理原则**：删掉破折号，用正常句子结构重写。真正的停顿用短句和段落断开实现，不靠标点。
...
- [ ] **描写不解释**：无直接定性句（"他很疲惫/气氛尴尬"），全用动作细节让读者自己得出结论
- [ ] **先否定再肯定句式**：每章"不是A，是B"结构不超过1次，总结段落禁用
- [ ] **破折号密度**：每500字不超过2个——，超出则检查是否用标点代替了写作
